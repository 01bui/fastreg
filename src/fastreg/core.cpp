...
int fastreg::cuda::getCudaEnabledDeviceCount() {
    int count;
...
void fastreg::cuda::setDevice(int device) { cudaSafeCall(cudaSetDevice(device)); }

std::string fastreg::cuda::getDeviceName(int device) {
    cudaDeviceProp prop;
...
inline void getCudaAttribute(T *attribute, CUdevice_attribute device_attribute, int device) {
    *attribute     = T();
    ...
    fastreg::cuda::error("driver API error", __FILE__, __LINE__);
}
...
void fastreg::cuda::printCudaDeviceInfo(int device) {
    ...
    for (int dev = beg; dev < end; ++dev) {
        ...
        /* not available in the CUDA Runtime API, so we make the necessary calls the driver API to support this for
         * output */
...
void fastreg::cuda::printShortCudaDeviceInfo(int device) {
    int count  = getCudaEnabledDeviceCount();
    bool valid = (device >= 0) && (device < count);
...
fastreg::ScopeTime::ScopeTime(const char *name_) : name(name_) { start = (double) cv::getTickCount(); }

fastreg::ScopeTime::~ScopeTime() {
    double time_ms = ((double) cv::getTickCount() - start) * 1000.0 / cv::getTickFrequency();
    std::cout << "time(" << name << ") = " << time_ms << "ms" << std::endl;
}

fastreg::SampledScopeTime::SampledScopeTime(double &time_ms) : time_ms_(time_ms) {
    start = (double) cv::getTickCount();
}

fastreg::SampledScopeTime::~SampledScopeTime() {
    static int i_ = 0;
    time_ms_ += getTime();
...
double fastreg::SampledScopeTime::getTime() {
